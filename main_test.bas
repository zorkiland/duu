...
{:mainloop_if_newpos}
	...
	'wenn nextpos
		...
		if c>=48 then {:raumaktion_npc}       'wenn c>=48
...
{:raumaktion_text}
	'aktion truhe
		...
		if c=12  and aa=1 then print"{home}{white}{down}{right}in der truhe ist was!"         :goto{:mainloop_oldpos}
	'aktion baum
		if c=25 and aa=0 then goto {:mainloop_oldpos}
		if c=25 and aa=1 then print"{home}{white}{down}{right}der baum sieht intressant aus! ":goto{:mainloop_oldpos}
{:raumaktion_npc}
	'npc                                                123456789a123456789b123456789c12345678
		if c=49 then print"{white}{home}{down:1}{right:2}ein heilzauber ist unter dem baum"     :goto{:mainloop_oldpos}
		if c=53 then print"{white}{home}{down:1}{right:2}ein feuerzauber ist in den katakomben" :goto{:mainloop_oldpos}
	'player
		if c=50 then print"{white}{home}{down:1}{right:2}ich komme mit dir!"                     :{var:npc_flag}(0)=1:gosub{:gosub_raumaktion_poke_mapspeicher}:{var:player_activ}(1)=1 :tx=12:ty=4:gosub{:gosub_raumaktion_print_one_tile}:gosub{:gosub_print_player_hp}:goto{:mainloop_oldpos}
		if c=52 then print"{white}{home}{down:1}{right:2}ich komme mit dir!"                     :{var:npc_flag}(1)=1:gosub{:gosub_raumaktion_poke_mapspeicher}:{var:player_activ}(2)=1 :tx=7 :ty=2:gosub{:gosub_raumaktion_print_one_tile}:gosub{:gosub_print_player_hp}:goto{:mainloop_oldpos}
		if c=51 then print"{white}{home}{down:1}{right:2}ich komme mit dir!"                     :{var:npc_flag}(2)=1:gosub{:gosub_raumaktion_poke_mapspeicher}:{var:player_activ}(3)=1 :tx=3 :ty=2:gosub{:gosub_raumaktion_print_one_tile}:gosub{:gosub_print_player_hp}:goto{:mainloop_oldpos}
	'monster
...
{:raumaktion_heilen}
	'wenn c=40 wasser
		if c=40 then print"{home}{white}{down}{right}du bist geheilt!": gosub{:gosub_print_player_hp}:goto{:mainloop_oldpos}
...
{:gosub_raumaktion_print_schalter_status}
	'schalter status
		if cr={var:schalter_raum}(0) and zx={var:schalter_posx}(0) and {var:schalter_flag}(0)=0 then print"{home}{white}{down}{right}der schalter ist aus!";:gosub {:gosub_delay_500}
		if cr={var:schalter_raum}(0) and zx={var:schalter_posx}(0) and {var:schalter_flag}(0)=1 then print"{home}{white}{down}{right}der schalter ist ein!";:gosub {:gosub_delay_500}
	return
...
{:untersuche}
	...
	'inventar max
		if is=99 then print"{home}{white}{down}{right}du kannst nicht mehr tragen!" : goto{:mainloop_oldpos}
	'loesche event item
		{var:event_raum}(ei)=-1
	'inventar print
		print"{home}{white}{down}{right}gefunden: ";{var:item_name}({var:inventar_slot}(is));: goto{:mainloop_oldpos}
	{:nimm_next}
		'wenn ei=max
		ei=ei+1
	...
	{:aktion_nichts}
		...
		'print "{home}{down}{right}{white}nichts besonderes!": goto{:mainloop_oldpos}
